...
CREATE TRIGGER checking BEFORE INSERT ON products FOR EACH ROW
BEGIN
  IF new.name IS NULL and new.description IS NULL
    THEN SIGNAL sqlstate '45001' set message_text = "product name or description shouldn't be NULL"; 
  end if;
END; //

CREATE checking BEFORE UPDATE ON products FOR EACH ROW
BEGIN
  IF new.name IS NULL and new.description IS NULL
    THEN SIGNAL sqlstate '45001' set message_text = "product name or description shouldn't be NULL"; 
  end if;
